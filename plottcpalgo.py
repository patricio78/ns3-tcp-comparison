...
def get_data(algo, variable='cwnd', duration=20.):
    ...
    for line in open(file_name, 'r'):
        data = np.append(
            data, np.array([list(map(float, line[:-1].split()))]),
            axis=0)
    data = data.T
...
if __name__ == '__main__':
    ...
    plot_cwnd_all_algorithms(duration=60.)
    print ("----- Plotting cwnd, ACK, and RTT of each algorithm -----")
    plot_cwnd_ack_rtt_each_algorithm(duration=60.)
